...
class FoodItems extends Component {
    ...
    getFoodList = () => {
        ...
        axios.post(Url.getfoodcategorieslisturl, 
        QueryString.stringify({
            restaurant_id : this.state.driverid,
        }), 
        {
            headers: {"Content-Type": "application/x-www-form-urlencoded",}
        }).then(response => {
            this.setState({foodlist : response.data.category});
    ...
    updateItem = (id, status) => {
        ...
        axios.post(Url.updatecateoryavailabilityurl, 
        QueryString.stringify({
            catid : id,
            status : status
        }), 
    ...
    render(){
        ...
        return(
            <View style={[styles.container]}>
                ...
                <Text style={[styles.pagetitle]}>{Languages.FoodCategories}</Text>
                <FlatList
                    itemDimension={80}
                    staticDimension={300}
                    fixed
...
export default function(props){
    ...
    return <FoodItems {...props} navigation={navigation} />;
}